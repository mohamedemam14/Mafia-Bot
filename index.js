...
const TOP_WEEK_ROOM_ID = "1460017456662712637"; // تأكد من صحة ID روم التوب
...
async function safeIncrementStats(channelId) {
...
async function safeSaveUserProgress(traineeId, updateFn) {
  return new Promise((resolve) => {
    queue.push(async () => {
      const data = loadProgress();
      if (!data[traineeId]) data[traineeId] = { courses: 0, events: 0, collab: 0, manualPoints: 0 };
...
async function updateStatsEmbed(client, statsData) {
  ...
  const embed = new EmbedBuilder().setTitle("📈 لوحة مراقبة الأداء العام").setColor(0x00ffcc).addFields(
  ...
  const msgs = await statsChannel.messages.fetch({ limit: 15 });
  const botMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title === "📈 لوحة مراقبة الأداء العام");
  if (botMsg) await botMsg.edit({ embeds: [embed] }); else await statsChannel.send({ embeds: [embed] });
}
...
async function updateTopWeekEmbed(client) {
  ...
  for (const [userId, userData] of Object.entries(data)) {
    ...
    const total = (userData.courses || 0) + (userData.events || 0) + (userData.collab || 0);
    if (total > 0) leaderboard.push({ userId, ...userData, total });
  }

  leaderboard.sort((a, b) => b.total - a.total);

  const embed = new EmbedBuilder()
    .setTitle("🏆 قائمة النشاط والتميز الأسبوعي")
    .setColor(0xF1C40F)
    .setThumbnail("https://cdn-icons-png.flaticon.com/512/3112/3112946.png")
    .setFooter({ text: "يتم التحديث تلقائياً عند اعتماد المهام" })
    .setTimestamp();
  ...
  if (leaderboard.length === 0) {
    ...
    for (let i = 0; i < Math.min(leaderboard.length, 10); i++) {
      ...
      const rankIcon = topThree[i] || "🔹";
      const rating = e.total >= 15 ? "💎 ممتاز" : (e.total >= 8 ? "✅ جيد جداً" : "✨ جيد");
      
      desc += `${rankIcon} <@${e.userId}>\n`;
      desc += `┃ 📚 الكورسات: \`${e.courses || 0}\` | 🎯 الفعاليات: \`${e.events || 0}\`\n`;
      desc += `┃ 🤝 التعاون: \`${e.collab || 0}\` | 📊 التقييم: ${rating}\n\n`;
    }
    embed.setDescription(desc);
    
    if (leaderboard.length > 0) {
        embed.addFields({ name: "🌟 نجم الأسبوع الحالي", value: `⭐ <@${leaderboard[0].userId}> بمجموع نقاط \`${leaderboard[0].total}\`` });
    }
  }

  const msgs = await topChannel.messages.fetch({ limit: 15 });
  const botMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title === "🏆 قائمة النشاط والتميز الأسبوعي");
  if (botMsg) await botMsg.edit({ embeds: [embed] }); else await topChannel.send({ embeds: [embed] });
}
...
client.on(Events.MessageCreate, async (message) => {
  // 1. حساب التعاون التلقائي (AUTO) وربطه بالتوب
  if (message.author.bot && AUTO_STATS_CHANNELS[message.channelId]) {
    const updatedStats = await safeIncrementStats(message.channelId);
    await updateStatsEmbed(client, updatedStats);

    const mentionedUser = message.mentions.users.first();
    if (mentionedUser) {
        await safeSaveUserProgress(mentionedUser.id, async (userData) => {
            userData.collab = (userData.collab || 0) + 1;
        });
    ...
    queue.push(async () => {
      ...
      for (const k in data) { 
        if (k !== 'stats') { 
            ...
            data[k].events = 0; 
            data[k].collab = 0; 
            data[k].manualPoints = 0;
        } 
      }
      fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
      await updateTopWeekEmbed(client);
      message.reply("✅ تم تصفير قائمة التوب بنجاح.");
    });
    processQueue();
    return;
  }

  // 3. إدارة رومات المهام والاعتماد اليدوي
  ...
  if (rank) {
    ...
    if (progress[message.author.id]?.[`rank${rank}`]?.completedRooms.includes(message.channelId)) {
        const warning = await message.reply("⛔ لقد أرسلت في هذه الغرفة سابقاً.");
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (!originalMessage) return interaction.reply({ content: "خطأ بالرسالة.", ephemeral: true });

  const traineeId = originalMessage.author.id;
  const roomId = interaction.channelId;

  if (interaction.customId === 'approve_task') {
    // تحديث الإحصائيات اليدوية ونقاط التوب
    if (MANUAL_STATS_CHANNELS[roomId]) {
      const updatedStats = await safeIncrementStats(roomId);
      await updateStatsEmbed(client, updatedStats);
