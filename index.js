...
const THEME = {
  ...
  night: "#000000",
  day: "#f1c40f",
  // يمكنك تغيير هذه الروابط لصور من اختيارك
  cover_img: "https://i.imgur.com/6Xy1Fk8.png", 
  win_mafia_img: "https://i.imgur.com/83pL6v6.png", 
  win_citizens_img: "https://i.imgur.com/kS9Yp9v.png" 
};

/* ================== بدء اللعبة ================== */
client.on("messageCreate", async msg => {
  if (msg.content !== "!mafia" || msg.author.bot) return;
  if (games.has(msg.guild.id)) return msg.reply("⚠️ هناك جولة دموية جارية بالفعل!");
  ...
  const embed = new EmbedBuilder()
    ...
    .setImage(THEME.cover_img)
    .setColor(THEME.main)
    .setFooter({ text: "تأكد من فتح الخاص لاستلام دورك السري" });
...
client.on("interactionCreate", async i => {
  ...
  if (i.customId === "join") {
    if (game.status !== "waiting") return i.reply({ content: "❌ بدأت اللعبة مسبقاً!", ephemeral: true });
    if (game.players.includes(i.user.id)) return i.reply({ content: "❌ أنت مسجل بالفعل!", ephemeral: true });
    game.players.push(i.user.id);
    return i.reply({ content: `🔥 انضممت للملحمة! العدد الحالي: ${game.players.length}`, ephemeral: true });
  }

  if (i.customId === "start") {
    if (i.user.id !== game.host) return i.reply({ content: "❌ المضيف فقط من يبدأ اللعبة!", ephemeral: true });
    if (game.players.length < 4) return i.reply({ content: "❌ نحتاج 4 لاعبين على الأقل!", ephemeral: true });
    
    game.status = "playing";
    game.alive = [...game.players];
    await i.reply({ content: "💀 جاري توزيع الأدوار.. تفقد رسائل الخاص فوراً!", ephemeral: true });
  ...
  if (i.customId === "open_night_menu") {
    ...
    return i.reply({ content: "⚔️ نفذ مهمتك السرية الآن (هذه الرسالة مخفية):", components: rows, ephemeral: true });
  }

  handleGameActions(i, game);
});
...
async function assignRolesAndNotify(game) {
  ...
  for (let i = 0; i < shuffled.length; i++) {
    ...
    let role, roleName, color, mission;

    if (i < mafiaCount) { 
        role = "mafia"; roleName = "مافيا 👺"; color = THEME.mafia; 
        mission = "اقتل الجميع بدم بارد ولا تترك أثراً لك."; 
    } else if (i === mafiaCount) { 
        role = "doctor"; roleName = "طبيب 🩺"; color = THEME.doctor; 
        mission = "حاول توقع ضحية المافيا وحمايتها كل ليلة."; 
    } else if (i === mafiaCount + 1) { 
        role = "police"; roleName = "محقق 🕵️"; color = THEME.police; 
        mission = "حلل الأدلة واكشف هوية الخونة كل ليلة."; 
    } else { 
        role = "citizen"; roleName = "مواطن 👤"; color = THEME.main; 
        mission = "ابقَ حياً وكافح لكشف المافيا في نقاشات الصباح."; 
    }

    game.roles[userId] = role;
    const user = await client.users.fetch(userId);
    const dmEmbed = new EmbedBuilder()
      .setTitle(`🃏 بطاقة هويتك: ${roleName}`)
      .setDescription(`مهمتك: **${mission}**`)
      .setColor(color)
      .setFooter({ text: "لا تشارك هذه الرسالة مع أحد!" });
...
async function startGameLoop(game) {
  while (game.status === "playing") {
    ...
    const nEmbed = new EmbedBuilder()
        ...
        .setDescription("المدينة نائمة.. أصحاب القلوب الميتة يتحركون الآن.")
        .setColor(THEME.night);
    const nRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("open_night_menu").setLabel("فتح قائمة الأهداف").setStyle(ButtonStyle.Secondary));
    ...
    const vEmbed = new EmbedBuilder()
        .setTitle("🗳️ اجتماع الصباح: من الخائن؟")
        .setDescription("تشاوروا جيداً.. صوت واحد قد ينهي حياة بريء أو يقتل مجرماً!")
        .setColor(THEME.day);
...
/* ================== الوظائف المساعدة ================== */
async function handleGameActions(i, game) {
    const [action, targetId] = i.customId.split("_");
    if (["kill", "save", "check"].includes(action)) {
        if (game.nightActions.has(i.user.id)) return i.reply({ content: "لقد نفذت قرارك بالفعل!", ephemeral: true });
        ...
        if (action === "check") {
            ...
            await i.update({ content: `🔍 تقرير المحقق: اللاعب **${member.displayName}** هو **${isM ? "خائن (مافيا) 👺" : "بريء ✅"}**`, components: [] });
        } else await i.update({ content: "✅ تم تسجيل قرارك السري بنجاح.", components: [] });
    }
    if (action === "vote") {
        ...
        await i.reply({ content: "🗳️ سجلت صوتك في الخفاء.", ephemeral: true });
    }
}
...
async function resolveNight(game) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(dead ? "💀 فاجعة في الصباح" : "✨ نجاة بأعجوبة")
        .setDescription(dead ? `استيقظت المدينة على خبر مقتل <@${dead}>.. المافيا لا ترحم!` : "حاولت المافيا الغدر، لكن الطبيب كان في المرصاد!")
...
async function resolveVote(game) {
    ...
    if (top) {
        ...
        const embed = new EmbedBuilder()
            .setTitle("⚖️ حكم المشنقة")
            .setDescription(`تقرر إعدام اللاعب <@${top}>.\n\nكشفت هويته الحقيقية أنه كان: **${role === 'mafia' ? 'خائناً (مافيا) 👺' : 'مواطناً بريئاً 👤'}**`)
            .setColor(THEME.main);
        game.lastMessages.push(await game.channel.send({ embeds: [embed] }));
    } else {
        game.lastMessages.push(await game.channel.send("🤝 لم يجمع أحد على ضحية.. نجا الجميع من المشنقة اليوم!"));
    }
    return checkWinner(game);
}
...
async function createPlayerRows(ids, prefix, guild) {
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        const name = member?.displayName || "مجهول";
...
function checkWinner(game) {
    ...
    if (m === 0) {
        const win = new EmbedBuilder()
            ...
            .setDescription(`تم سحق المافيا وتطهير المدينة! \n\n**الأبطال الصامدون:**\n${game.alive.map(id => `<@${id}>`).join(", ")}`)
    ...
    if (m >= c) {
        const win = new EmbedBuilder()
            .setTitle("👺 سيادة المافيا!")
            .setDescription(`سقطت المدينة رسمياً في قبضة الخيانة! \n\n**قادة المافيا المنتصرين:**\n${game.players.filter(p => game.roles[p] === 'mafia').map(id => `<@${id}>`).join(", ")}`)
