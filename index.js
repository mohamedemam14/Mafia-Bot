...
async function safeSaveUserProgress(traineeId, updateFn) {
  return new Promise((resolve) => {
    queue.push(async () => {
      const data = loadProgress();
      if (!data[traineeId]) data[traineeId] = { courses: 0, events: 0, collab: 0 };
...
async function updateStatsEmbed(client, statsData) {
  ...
  const msgs = await statsChannel.messages.fetch({ limit: 20 });
...
async function updateTopWeekEmbed(client) {
  ...
  for (const [userId, userData] of Object.entries(data)) {
    ...
    if (total > 0) leaderboard.push({ userId, courses: userData.courses || 0, events: userData.events || 0, collab: userData.collab || 0, total });
  }
  leaderboard.sort((a, b) => b.total - a.total);

  const embed = new EmbedBuilder().setTitle("🏆 قائمة النشاط والتميز").setColor(0xF1C40F).setTimestamp();
  if (leaderboard.length === 0) { embed.setDescription("لا توجد بيانات مسجلة."); } else {
    ...
    for (let i = 0; i < Math.min(leaderboard.length, 10); i++) {
      ...
      desc += `${i + 1}. **${name}**\n┃ 📚 كورسـات: \`${e.courses}\` | 🎯 فعاليات: \`${e.events}\` | 🤝 تعاون: \`${e.collab}\`\n┃ التقييم: ${rating}\n\n`;
    }
    embed.setDescription(desc);
  }
  const msgs = await topChannel.messages.fetch({ limit: 20 });
  const botMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title === "🏆 قائمة النشاط والتميز");
...
client.on(Events.MessageCreate, async (message) => {
  // 1. حساب التعاون التلقائي (AUTO) - يتم احتسابه بمجرد إرسال البوت في الغرف المحددة
  if (message.author.bot && AUTO_STATS_CHANNELS[message.channelId]) {
    // تحديث لوحة الإحصائيات العامة (تعاون الأقسام)
    const updatedStats = await safeIncrementStats(message.channelId);
    await updateStatsEmbed(client, updatedStats);

    // محاولة ربط التعاون بالعضو لزيادة نقاطه في التوب (إذا وجد منشن في الرسالة)
  ...
  if (message.content === "!reset" && message.member.roles.cache.has(ADMIN_ROLE_ID)) {
    queue.push(async () => {
      const data = loadProgress();
      for (const k in data) { if (k !== 'stats') { data[k].courses = 0; data[k].events = 0; data[k].collab = 0; } }
      fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
  ...
  // 3. إدارة رومات المهام والتقارير اليدوية
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId === 'approve_task') {
    // تحديث التوب اليدوي (كورسات/فعاليات)
    ...
    await safeSaveUserProgress(traineeId, async (userData) => {
      ...
      if (!data.completedRooms.includes(roomId)) {
        ...
        if (followChannel) {
          ...
          if (exMsg) await exMsg.edit({ content }); else { const n = await followChannel.send({ content }); data.followMessageId = n.id; }
        }
