...
// نظام التفاعل (الريأكشن) لفتح لوحة التحكم أو زيادة إحصائيات الجدد
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  // تفعيل لوحة التحكم للتقارير عند التفاعل
  const isRankTask = TASKS_RANK_2[reaction.message.channelId] || TASKS_RANK_3[reaction.message.channelId];
  const isManualTask = MANUAL_STATS_CHANNELS[reaction.message.channelId];

  if (isRankTask || isManualTask) {
    // التحقق من أن لوحة التحكم لم تفتح بالفعل لهذا التقرير
    const messages = await reaction.message.channel.messages.fetch({ limit: 5 });
    const alreadySent = messages.some(m => m.content.includes("تحكم الإدارة لتقرير") && m.content.includes(reaction.message.author.id));
...
client.on(Events.InteractionCreate, async (interaction) => {
  if (interaction.isButton()) {
    ...
    // استخراج الرسالة الأصلية من الريفرنس أو المحتوى
    const originalMessageId = interaction.message.reference?.messageId;
    const originalMessage = originalMessageId ? await interaction.channel.messages.fetch(originalMessageId).catch(() => null) : null;
  ...
  if (interaction.isModalSubmit()) {
    ...
    const msgId = parts[2] === 'task' ? parts[3] : parts[2]; 
