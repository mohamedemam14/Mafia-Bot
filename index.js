...
const TOP_WEEK_ROOM_ID = "1459162751288217869"; // يفضل تغييره لروم مستقل
...
async function processQueue() {
  ...
  finally { isWriting = false; processQueue(); }
}
...
async function updateTopWeekEmbed(client) {
  ...
  if (leaderboard.length === 0) {
    embed.setDescription("لا توجد بيانات متاحة لهذا الأسبوع.");
  } else {
    let listContent = "";
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (message.content === "!reset" && message.member.roles.cache.has(ADMIN_ROLE_ID)) {
    queue.push(async () => {
      ...
      fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
      message.reply("✅ تم تصفير نقاط توب الأسبوع بنجاح.");
  ...
  if (rank) {
    ...
    if (progress[message.author.id]?.[`rank${rank}`]?.completedRooms.includes(message.channelId)) {
      const warn = await message.reply("⛔ لقد أنهيت هذه المهمة مسبقاً.");
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (!originalMessage) return interaction.reply({ content: "خطأ: لم يتم العثور على الرسالة الأصلية.", ephemeral: true });
      ...
      await safeSaveUserProgress(traineeId, async (userData) => {
        ...
        if (!data.completedRooms.includes(roomId)) {
          ...
          if (followChannel) {
            ...
            const content = `### 📑 مـلف تـدريب (Rank ${rank})\n**المتدرب:** <@${traineeId}>\n**المهام:** ${data.tasks.length}/${allTasks.length}`;
            if (data.followMessageId) {
              ...
              else { const newMsg = await followChannel.send({ content }); data.followMessageId = newMsg.id; }
            } else {
          ...
          if (data.tasks.length === Object.keys(rank === 2 ? TASKS_RANK_2 : TASKS_RANK_3).length && !data.upgradeNotified) {
            ...
            if (rRoom) await rRoom.send(`🎊 <@${traineeId}> جاهز للترقية لرتبة Rank ${rank}`);
          }
        }
      });
    }
